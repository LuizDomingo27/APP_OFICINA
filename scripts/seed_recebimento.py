...
seed_recebimento.py
====================
Migração única (seed) da base de Recebimento: lê o `.xlsx` que já está
salvo em `database/recebimento.xlsx` (mecanismo antigo, ver
`src/shared/database.py`), aplica a MESMA normalização de negócio que o
app já usa (`_normalize_raw`, em `src/recebimento/data_loader.py`) e
grava o resultado em `database/app.db` (SQLite, ver `src/shared/sql_store.py`).

Roda fora do Streamlit de propósito (nenhuma das funções chamadas aqui
depende de `st.cache_data` ou de um app rodando) — é só:

    python scripts/seed_recebimento.py
...
Importante: este script NÃO apaga `database/recebimento.xlsx` — o
...
# Permite rodar como `python scripts/seed_recebimento.py` a partir da raiz do projeto
ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(ROOT))

import pandas as pd

from src.recebimento.data_loader import _normalize_raw, DataLoadError
from src.shared import sql_store

XLSX_ANTIGO = ROOT / "database" / "recebimento.xlsx"
...
def main() -> int:
    ...
    print("Gravando em database/app.db (tabela 'recebimento')...")
    sql_store.substituir_tabela(
        "recebimento", df_limpo, indices=["DIA", "MP", "OFICINA"]
    )
    sql_store.registrar_metadata("recebimento", XLSX_ANTIGO.name)

    print("Validando leitura de volta do banco...")
    df_volta = sql_store.carregar_tabela("recebimento")
